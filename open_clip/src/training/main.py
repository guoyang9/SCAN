...
from open_clip import create_model_and_transforms, trace_model, get_tokenizer, create_loss, PrePrune, CosineAnnealing
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if prune_info['aft_warmup']:
            ...
            if epoch_diff % (cos_alg.T_cos+1) == 0: # dump the prune set
                is_mutate = False
                dump_set = None
                prune_set = {'red': set(), 'ill': set(),}
            else: # utilize the prune set
                is_mutate = True 
                ...
                if is_master(args):
                    logging.info(f'Using prune set with size {len(dump_set)}')   
                
            # very important to reset the dataset for the new epoch
            if dump_set is not None:
                args.train_num_samples -= len(dump_set)
            data = get_data(
                args,
                (preprocess_train, preprocess_val),
                epoch=epoch,
            ...
            if dump_set is not None: # add the dumped set back after the data is loaded
                args.train_num_samples += len(dump_set)
                
        prune_pre_local = prune_pre if (prune_info['aft_warmup'] and not is_mutate) else None
        output_set, loss_cur = train_one_epoch(model, data, loss, epoch, optimizer, 
                                               scaler, scheduler, dist_model, args, 
                                               tb_writer=writer,
        ...
        if not prune_info['aft_warmup']:
            # if is_master(args):
            #     logging.info(f'Epoch {epoch} loss: {loss_cur}, loss_pre: {prune_info["loss_pre"]}')
            if (prune_info['loss_pre'] - loss_cur) / (prune_info['loss_pre'] + 1e-8) < args.prune_td:
                prune_info['aft_warmup'] = True
                prune_info['aft_warmup_epoch'] = epoch + 1
                if is_master(args):
                    logging.info(f'Start prune from epoch {epoch + 1}.')
            else:
                prune_info['loss_pre'] = loss_cur
